...
namespace TinyDIP
{
    ...
    template <typename T, std::size_t N> struct is_unsigned_data<TinyDIP::MultiChannel<T, N>> : is_unsigned_data<T> {};
    ...
    template<   typename F, 
                ...
                std::size_t N,
                typename... Ts>
    requires std::invocable<F, Container<T, N>, Ts...>
    struct recursive_array_invoke_result<0, F, Container<T, N>, Ts...>
    {
        using type = std::invoke_result_t<F, Container<T, N>, Ts...>;
    };
    ...
    */
    ...
    requires (std::is_execution_policy_v<std::remove_cvref_t<ExPo>> &&
              ...
              requires { typename recursive_invoke_result_t<unwrap_level, F, T>; })
    ...
    //  log10 Template Function Implementation (the version with execution policy)
    template<class ExecutionPolicy, typename T>
    requires (std::is_execution_policy_v<std::remove_cvref_t<ExecutionPolicy>>)
    [[nodiscard]] constexpr static auto log10(ExecutionPolicy&& execution_policy, const T& input)
    {
        if constexpr (Multichannel<T>)
        {
            return apply_multichannel(
                std::forward<ExecutionPolicy>(execution_policy),
                input,
                [&](auto&& _input) {return log10(std::forward<ExecutionPolicy>(execution_policy), _input); }
            );
        }
        else
        {
    ...
    //  log1p Template Function Implementation
    template<typename T>
    [[nodiscard]] constexpr static auto log1p(const T& input)
    {
        if constexpr (Multichannel<T>)
        {
            return apply_multichannel(input, [&](auto&& _input) {return log1p(_input); });
        }
        else
        {
    ...
    //  log1p Template Function Implementation (the version with execution policy)
    template<class ExecutionPolicy, typename T>
    requires (std::is_execution_policy_v<std::remove_cvref_t<ExecutionPolicy>>)
    [[nodiscard]] constexpr static auto log1p(ExecutionPolicy&& execution_policy, const T& input)
    {
        if constexpr (Multichannel<T>)
        {
            return apply_multichannel(
                std::forward<ExecutionPolicy>(execution_policy),
                input,
                [&](auto&& _input) {return log1p(std::forward<ExecutionPolicy>(execution_policy), _input); }
            );
        }
        else
        {
    ...
    //  log2 Template Function Implementation (the version with execution policy)
    template<class ExecutionPolicy, typename T>
    requires (std::is_execution_policy_v<std::remove_cvref_t<ExecutionPolicy>>)
    [[nodiscard]] constexpr static auto log2(ExecutionPolicy&& execution_policy, const T& input)
    {
        if constexpr (Multichannel<T>)
        {
            return apply_multichannel(
                std::forward<ExecutionPolicy>(execution_policy),
                input,
                [&](auto&& _input) {return log2(std::forward<ExecutionPolicy>(execution_policy), _input); }
            );
        }
        else
        {
    ...
    template <std::size_t N, class ExPo, typename TupleT, class ElementT = std::tuple_element_t<N, TupleT>, class Function = std::plus<ElementT>>
    requires(std::is_execution_policy_v<std::remove_cvref_t<ExPo>> and
             std::regular_invocable<Function, ElementT, ElementT> and
             (N < std::tuple_size_v<TupleT>))
    constexpr static auto sum_n_th_element(ExPo&& execution_policy, const std::vector<TupleT>& tuples, const ElementT initVal = ElementT{}, const Function& f = Function{})
    {
        const std::size_t size = tuples.size();
        std::vector<ElementT> elements(size);
        std::transform(std::forward<ExPo>(execution_policy), std::ranges::cbegin(tuples), std::ranges::cend(tuples), std::ranges::begin(elements),
                       [&](const auto& tup) { return std::get<N>(tup); });
