...
//  MaxHandler struct implementation
struct MaxHandler
{
    std::shared_ptr<Workspace> workspace_;
    ...
    constexpr void operator()(const ArgsT& args, std::ostream& os = std::cout, ImageLoaderFun&& image_loader_fun = ImageLoaderFun{}) const
    {
        if (std::ranges::empty(args))
        {
            os << "Usage: max <input_img | $var> [output_var | $var]\n";
            return;
        }
        ...
        os << "Calculating max of " << input_arg << "...\n";

        // Polymorphic lambda to cleanly execute the algorithm dynamically independent of image type
        auto process_max = [&]<typename ImageType>(ImageType&& input_img)
        {
            auto handle_result = [&](auto&& max_result)
            {
                if (!std::ranges::empty(output_arg))
                {
                    if (output_arg.starts_with('$'))
                    {
                        workspace_->store(output_arg.substr(1), max_result);
                        os << "Saved max result to " << output_arg << "\n";
                    }
                    else
                    {
                        os << "Error: Output must be a memory variable starting with '$'.\n";
                ...
                {
                    if constexpr (requires { os << max_result; })
                    {
                        if constexpr (sizeof(decltype(max_result)) == 1 && std::is_integral_v<std::decay_t<decltype(max_result)>>)
                        {
                            os << "Max result: " << +max_result << "\n";
                        }
                        else
                        {
                            os << "Max result: " << max_result << "\n";
                    ...
                    {
                        os << "Max result evaluated successfully (Non-printable complex type).\n";
                    }
                }
            };

            handle_result(TinyDIP::max(std::forward<ImageType>(input_img)));
        };
        ...
        if (input_arg.starts_with('$'))
        {
            ...
            if (workspace_->retrieve<TinyDIP::Image<TinyDIP::RGB>>(var_name))
            {
                process_max(image_loader_fun.template operator()<TinyDIP::Image<TinyDIP::RGB>>(input_arg, workspace_));
            }
            else if (workspace_->retrieve<TinyDIP::Image<double>>(var_name))
            {
