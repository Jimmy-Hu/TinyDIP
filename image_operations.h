...
namespace TinyDIP
{
    ...
    */
                ...
                for (std::size_t inner_z = 0; inner_z < N3; ++inner_z)
                {
                    ...
                    for (std::size_t inner_y = 0; inner_y < plane.getHeight(); ++inner_y)
                    {
                        for (std::size_t inner_x = 0; inner_x < plane.getWidth(); ++inner_x)
                        {
                            ...
                            OutputT alpha1 = (inner_x == 0) ? (std::numbers::sqrt2_v<OutputT> / 2) : (static_cast<OutputT>(1.0));
                            OutputT alpha2 = (inner_y == 0) ? (std::numbers::sqrt2_v<OutputT> / 2) : (static_cast<OutputT>(1.0));
                            OutputT alpha3 = (inner_z == 0) ? (std::numbers::sqrt2_v<OutputT> / 2) : (static_cast<OutputT>(1.0));
    ...
    template<arithmetic ElementT = double, arithmetic OutputT = ElementT>
    constexpr std::vector<Image<OutputT>> idct3(const std::vector<Image<ElementT>>& input)
    {
        std::vector<Image<OutputT>> output;
        output.reserve(input.size());
        for (std::size_t i = 0; i < input.size(); ++i)
        {
            output.emplace_back(idct3_detail(input, i));
        }
        return output;
    }
    ...
    template<class ExPo, TinyDIP::arithmetic ElementT = double>
    requires (std::is_execution_policy_v<std::remove_cvref_t<ExPo>>)
