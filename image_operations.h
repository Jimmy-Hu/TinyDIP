...
namespace TinyDIP
{
    ...
    */
    ...
    //  rgb2hsv function implementation
    inline auto rgb2hsv(RGB_DOUBLE input)
    {
    ...
    //  Grayscale2RGB function implementation
    inline auto Grayscale2RGB(const Image<GrayScale>& input)
    {
    ...
    template<class ElementT, class F, class... Args>
    constexpr static auto apply_each_single_output(const std::size_t channel_count, const Image<ElementT>& input1, F operation, Args&&... args)
    {
        std::vector<decltype(std::invoke(operation, getPlane(input1, 0), args...))> output;
        output.reserve(channel_count);
        for (std::size_t channel_index = 0; channel_index < channel_count; ++channel_index)
        {
            output.emplace_back(std::invoke(operation, getPlane(input1, channel_index), args...));
        }
        return output;
    }
    ...
    inline auto im2double(Image<RGB> input)
    {
    ...
    inline auto im2uint8(Image<RGB_DOUBLE> input)
    {
    ...
    inline void print_with_latex_to_file(Image<RGB> input, std::string filename)
    {
    ...
    */
    ...
    // lanczos_resample template function implementation
    // Lanczos resampling function for 2D images
    // ExecutionPolicy: The execution policy (e.g., std::execution::seq, std::execution::par)
    // input_image: The original image to be resampled
    // new_width: The target width of the resampled image
    // new_height: The target height of the resampled image
    // a: The Lanczos kernel size parameter (default is 3 for high quality)
    template<class ExecutionPolicy, arithmetic ElementT>
    requires std::is_execution_policy_v<std::remove_cvref_t<ExecutionPolicy>>
        ...
        const int a = 3)
    {
        ...
        {
            // Simply forward each plane natively! The underlying single-plane
            // overload is now fortified to safely elevate to double precision
            // implicitly and clamp back down precisely to the original element type.
    ...
    inline Image<RGB> subtract(const Image<RGB>& input1, const Image<RGB>& input2)
    {
        check_size_same(input1, input2);
        return pixelwise_transform(
                [](RGB x, RGB y)
    ...
    */
    ...
    template<typename ElementT = double>
    constexpr static auto sin(const Image<ElementT>& input)
    {
        if constexpr ((std::same_as<ElementT, RGB>) || (std::same_as<ElementT, RGB_DOUBLE>) || (std::same_as<ElementT, HSV>) || is_MultiChannel<ElementT>::value)
        {
            return apply_each(input, [&](auto&& planes) { return sin(planes); });
        }
        else
        {
            Image<ElementT> output_image(
                TinyDIP::recursive_transform<1>([](auto&& _input) { return std::sin(_input); }, input.getImageData()),
                input.getSize()
            );
            return output_image;
        }
    ...
    template<typename ElementT = double>
    constexpr static auto cos(const Image<ElementT>& input)
    {
        if constexpr ((std::same_as<ElementT, RGB>) || (std::same_as<ElementT, RGB_DOUBLE>) || (std::same_as<ElementT, HSV>) || is_MultiChannel<ElementT>::value)
        {
            return apply_each(input, [&](auto&& planes) { return cos(planes); });
        }
        else
        {
            Image<ElementT> output_image(
                TinyDIP::recursive_transform<1>([](auto&& _input) { return std::cos(_input); }, input.getImageData()),
                input.getSize()
            );
            return output_image;
        }
    ...
    template<typename ElementT = double>
    constexpr static auto csc(const Image<ElementT>& input)
    {
        if constexpr ((std::same_as<ElementT, RGB>) || (std::same_as<ElementT, RGB_DOUBLE>) || (std::same_as<ElementT, HSV>) || is_MultiChannel<ElementT>::value)
        {
            return apply_each(input, [&](auto&& planes) { return csc(planes); });
        }
        else if constexpr (is_complex<ElementT>::value)
        {
            Image<ElementT> output_image(
                TinyDIP::recursive_transform<1>([&](auto&& _input) { return std::complex{ static_cast<ElementT>(1) } / std::sin(_input); }, input.getImageData()),
                input.getSize()
            );
            return output_image;
        }
        ...
        {
            Image<ElementT> output_image(
                TinyDIP::recursive_transform<1>([&](auto&& _input) { return 1 / std::sin(_input); }, input.getImageData()),
                input.getSize()
            );
            return output_image;
        }
    ...
    template<typename ElementT = double>
    constexpr static auto acos(const Image<ElementT>& input)
    {
        if constexpr ((std::same_as<ElementT, RGB>) || (std::same_as<ElementT, RGB_DOUBLE>) || (std::same_as<ElementT, HSV>) || is_MultiChannel<ElementT>::value)
        {
            return apply_each(input, [&](auto&& planes) { return acos(planes); });
        }
        else
        {
            Image<ElementT> output_image(
                TinyDIP::recursive_transform<1>([&](auto&& _input) { return std::acos(_input); }, input.getImageData()),
                input.getSize()
            );
            return output_image;
        }
